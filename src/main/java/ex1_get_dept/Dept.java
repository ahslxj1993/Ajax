...
/*
 ...
 자바빈(JavaBean)은 자바빈 규약에 따라 작성된 자바클래스 입니다
 예를들어 회원정보, 게시판 글 등의 정보를 출력할때 정보를 저장하고 있는 자바빈 객체를 사용하게 됩니다.
 
 *다음은 자바빈의 규약입니다
 1.필드마다 별도의 get/set 메서드가 존재해야 한다
 2.get 메서드는 파라미터가 존재하지 않아야 한다
 3.set 메서드는 반드시 하나이상의 파라미터가 존재해야 한다.
 4.빈즈 컴포넌트의 속성은 반드시 읽기 또는 쓰기가 가능해야 한다.
 	단, 읽기 전용인 경우 get 또는 is 메서드만 정의할수 있다.
 ...
 *자바빈 클래스는 데이터를 저장하는 필드, 데이터를 읽어올때 사용되는 메서드(getter 메서드),
 	값을 저장할때 사용되는 메서드(setter 메서드)로 구성됩니다
 
 *메서드 이름을 사용해서  프로퍼티(property)의 이름을 결정하게 됩니다.
  예를들어 프로퍼티의 이름이 name이고 값의 타입이 int 인 경우 프로퍼티와 관련된 메서드의 이름은 다음과 같이 결정됩니다.
  public void setName(int value){};
  public int getName(){};
  
  setName()과 getName()을 통해서 관리되는 데이터를 프로퍼티(property)라고 부릅니다
  
  프로퍼티의 타입이 boolean인 경우 get대신 is를 붙일수 있습니다
 ...
 *** 데이터 빈 (Data Bean) 클래스 작성 ***
 	...
 	DB에서 만들었던 컬럼 명과 동일하게 프로퍼티들을 생성합니다
*/
public class Dept {
	private int deptno;
